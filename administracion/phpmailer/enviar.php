<?php
   ...
   //Recogemos el destinatario del mensaje y el identificador de la consulta
   $destinatario=$_POST['email_destinatario'];
   $identificador_consulta=$_POST['idconsulta'];
   
   // Importamos la clases phpmailer
   ...
   if(isset($_POST['titulo']) && isset($_POST['respuesta'])){
   
       //Recogemos los datos del formulario
       $titulo=$_POST['titulo'];
       $respuesta=$_POST['respuesta'];
   
       //Fecha en la que se constesto la respuesta
       $fecha_respuesta= date("Y-m-d");
   
        // Pasandole true, activamos la excepciones
         ...
         <div class="col-sm-8" style="width: 100%" align="center">
            <?php
               try {
                 ...
                 $mail->Username = 'apphinojos@gmail.com';                 // SMTP username
                 $mail->Password = 'Prueba_1989*';                           // SMTP password
                 $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
                 $mail->Port = 587;                                    // TCP port to connect to
                 
                 //Emisor y receptor
                 $mail->setFrom('apphinojos@gmail.com', 'Ayuntamiento de Hinojos');
                 ...
                 echo '<h3>El email ha sido enviado a'. $destinatario . ' </br></h3>';
                 
                     //Compruebo que nos hemos conectado correctamente a la bd y actualizo la consulta a RESPONDIDA y almaceno la respuesta
                     if ($conexion) {
                         $resultado = mysqli_query($conexion," UPDATE consultas SET resuelta=1, respuesta='$respuesta', fecha_respuesta='$fecha_respuesta'  WHERE id='$identificador_consulta' ");
                     }
               
               
               } catch (Exception $e) {
                 echo '<img src="../imagenes/error.jpg" /><br/>';
                 echo '<h3>El email no ha podido ser enviado: ', $mail->ErrorInfo.'</h3>';
               } 
            ?>
         </div>
      </div>
